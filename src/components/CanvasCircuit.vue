...
export default {
  ...
  methods: {
    ...
    fun_circuitFig1(w, h, x, y) {
      ...
      */
      ...
      ctx.beginPath();
      // 连接点1
      if (this.schematics_data.inConv_DIGIN === 1) {
        //连接
        ctx.moveTo(
          rectangle4_xu_x +
          rectangle4_xu_w -
  ...
  // 挂载后
  mounted() {
    this.signal_val(true);
    this.isVisible = document.hidden ? false : true; // 初始化可见性状态
    // 设置页面可见性变化时的监听器
    document.addEventListener("visibilitychange", () => {
      ...
      if (!this.isVisible && this.intervalId !== null) {
        ...
        this.intervalId = null; // 重置intervalId，表示定时器已清除
      } else if (this.isVisible && this.intervalId === null) {
        // 如果页面重新变为可见，并且定时器已清除，则重新启动定时器
        this.startInterval();
      }
    });
